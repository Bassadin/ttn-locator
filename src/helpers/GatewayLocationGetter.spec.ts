...
describe('Test GatewayLocationGetter class', () => {
    test(`Test a real gateway's location`, async () => {
        ...
        const gatewayLocation = await GatewayLocationGetter.getGatewayLocation('eui-ac1f09fffe06048e');
    ...
    test(`Test calling the method with an invalid ID`, async () => {
        await expect(GatewayLocationGetter.getGatewayLocation('invalid_id')).rejects.toThrowError();
    });
});
